...
export default class BrowserFileSystemProvider implements FileSystemProvider {
  *getDirectoryTree(path: string = "/", params: any = {}): Generator<string, void, unknown> {
    ...
    for (const filePath of matchingFiles) {
      const relativePath = normalizedPath === "/" ? filePath.substring(1) : filePath.substring(normalizedPath.length + 1);
  ...
  createDirectory(_path: string, _options?: { recursive?: boolean | undefined }): boolean {
    //noop
    return true;
  }
  ...
  writeFile(filePath: string, content: string | Buffer): boolean {
    mockFileSystem[filePath] = { content: content.toString("utf-8") };
    return true;
  }

  appendFile(filePath: string, content: string | Buffer): boolean {
  ...
  copy(source: string, destination: string, options: { overwrite?: boolean | undefined } = {}): boolean {
    console.warn(`BrowserFileSystemService: copy called from ${source} to ${destination}. This is a mock implementation.`);
    ...
    if (mockFileSystem[destination] && !options.overwrite) {
      throw new Error(`Destination file already exists: ${destination}. Use overwrite option to replace.`);
    }
  ...
  rename(oldPath: string, newPath: string): boolean {
    console.warn(`BrowserFileSystemService: rename called from ${oldPath} to ${newPath}. This is a mock implementation.`);
  ...
  grep(searchString: string | string[], options?: GrepOptions): GrepResult[] {
    ...
    for (const [file, { content }] of Object.entries(mockFileSystem)) {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(search)) {
          ...
          const contextContent = linesBefore > 0 || linesAfter > 0 ? lines.slice(start, end + 1).join("\n") : null;
